...
//modif
app.put("/update/mesDonnes/:id", (req, res) => {
  const id = req.params.id;
    const body = req.body;
  console.log(id)
  const queryString =`UPDATE user SET  name =? , nom = ? ,prenom=?, email =? ,telephone= ?,societe= ? WHERE id =?`

  db.query(queryString,[body.name,body.nom,body.prenom,body.email,body.telephone,body.societe,id],(err, result) => {
    if (err) {
      console.log(err);
      return;
    }
